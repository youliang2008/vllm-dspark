...
"""DSpark speculative decoding evaluation: acceptance rate & speedup.

Runs under the V2 model runner (DSpark inference exists only there) with
``method="dspark"``.
"""

import os

# DSpark inference is only available in the V2 model runner; set before vLLM import.
...
def load_prompts(data_path: str, num_samples: int) -> list[str]:
    prompts = []
...
def run_dspark(args, prompts: list[str]) -> dict:
    print("\n" + "=" * 60)
    print("  DSpark Speculative Decoding")
    print("=" * 60)
    llm = LLM(
        model=args.target_model,
        tensor_parallel_size=args.tensor_parallel_size,
    ...
    print(
        f"\n[DSpark] time={elapsed:.1f}s tokens={total_tokens} "
...
def main():
    parser = argparse.ArgumentParser(description="DSpark eval")
    ...
    dspark_result = run_dspark(args, prompts)

    speedup = (
        ar_result["time_s"] / dspark_result["time_s"]
        if dspark_result["time_s"] > 0
        else 0
    )
    tps_speedup = (
        dspark_result["tokens_per_second"] / ar_result["tokens_per_second"]
    ...
    print(f"  DSpark throughput: {dspark_result['tokens_per_second']:.1f} tok/s")
    print(f"  Wall-clock speedup: {speedup:.2f}x")
    print(f"  Throughput speedup: {tps_speedup:.2f}x")
    print(f"  Acceptance rate:    {dspark_result['acceptance_rate']:.3f}")
    print(f"  Acceptance length:  {dspark_result['acceptance_length']:.2f}")
    ...
    summary = {
        ...
        "ar": ar_result,
        "dspark": dspark_result,
        "wall_clock_speedup": speedup,
        "throughput_speedup": tps_speedup,
    }
    output_path = args.output or f"dspark_eval_{args.num_samples}samples.json"
