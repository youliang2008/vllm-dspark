...
"""Stage 4: DDP training loop for the vLLM-native DFlash draft.

The draft is small (~0.5B params), so plain DistributedDataParallel across the
8 GPUs is used for data parallelism (the memory-heavy embed/lm_head are frozen
and target-shared, so FSDP sharding would give no benefit). Loss is a chunked
cross-entropy over the parallel block predictions.

Launch (see scripts/train.sh):
    torchrun --nproc_per_node=8 -m dflash_training.train
"""
...
from .dataset import DFlashBatchDataset
from .dflash_draft_model import (
    DFlashDraftForTraining,
    block_cross_entropy,
    block_hybrid_loss,
)
...
def _load_frozen_tensor(cfg: DFlashConfig, key: str, device) -> torch.Tensor:
    """Load a single named tensor from the target checkpoint (via the index)."""
...
def _lr_at(step: int, cfg: DFlashConfig) -> float:
...
def main() -> None:
    ...
    # ---- frozen target embed_tokens + lm_head (shared, not trained) --------
    ...
    # ---- model -------------------------------------------------------------
    model = DFlashDraftForTraining(cfg).to(device=device, dtype=torch.bfloat16)
    ...
    # ---- data --------------------------------------------------------------
    dataset = DFlashBatchDataset(
        cfg, rank=rank, world_size=world_size, seed=cfg.seed
    )
    loader = DataLoader(
        dataset,
        batch_size=None,
    ...
    while step < cfg.max_steps:
        ...
        # target_last_hidden is optional (requires re-extraction with
        # EXTRACT_TARGET_LAST_HIDDEN=1). Only needed for L1 loss.
        target_last_hidden = None
        if "target_last_hidden" in batch and cfg.l1_loss_alpha > 0:
            target_last_hidden = batch["target_last_hidden"].to(
        ...
        loss, metrics = block_hybrid_loss(
            hidden,
            ...
            ce_loss_alpha=cfg.ce_loss_alpha,
            l1_loss_alpha=cfg.l1_loss_alpha,
            loss_decay_gamma=cfg.loss_decay_gamma,
        )
